...
class _SfCalendarState extends State<SfCalendar> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _currentDate =
        DateTimeHelper.getDateTimeValue(getValidDate(widget.minDate, widget.maxDate, _controller.displayDate ?? widget.initialDisplayDate));
          ...
          if (widget.monthViewSettings.showAgenda) {
            ...
            if (position.dy > _minHeight - agendaHeight) {
              ...
              final List<dynamic> selectedAppointments =
                  _getSelectedAppointments(Offset(position.dx, position.dy - (_minHeight - agendaHeight)), currentSelectedDate);
      ...
      case CalendarView.timelineMonth:
        {
          ...
          return (_customScrollViewKey.currentWidget! as CustomCalendarScrollView)
              .getCalendarDetails(Offset(_isRTL ? updatedPosition.dx : updatedPosition.dx - resourceViewSize, updatedPosition.dy));
        }
  ...
  CalendarDetails? _getScheduleViewDetails(Offset position) {
    ...
    bool isNeedInBetweenMonthBuilder =
        _useMobilePlatformUI && (startDate.month != (isSameOrBeforeDate(_maxDate, endDate) ? endDate : _maxDate!).month);
    ...
    for (int i = 0; i < dateAppointmentKeys.length; i++) {
      ...
      final double panelHeight =
          totalPadding + ((eventsCount - allDayEventCount) * appointmentViewHeight) + (allDayEventCount * allDayAppointmentHeight);
      ...
      if (topPosition <= yPosition && yPosition < topPosition + panelHeight + appointmentViewPadding + dividerHeight) {
        ...
        currentAppointments
            .sort((CalendarAppointment app1, CalendarAppointment app2) => AppointmentHelper.orderAppointmentsAscending(app1.isAllDay, app2.isAllDay));
      ...
      case CalendarView.month:
        {
          if (isSameDate(_currentDate, _controller.displayDate) ||
              (isDateWithInDateRange(
                      _currentViewVisibleDates[0], _currentViewVisibleDates[_currentViewVisibleDates.length - 1], _controller.displayDate) &&
  ...
  void _updateCurrentVisibleDates() {
    final List<int>? nonWorkingDays =
        (_view == CalendarView.workWeek || _view == CalendarView.timelineWorkWeek) ? widget.timeSlotViewSettings.nonWorkingDays : null;
    final int visibleDatesCount = DateTimeHelper.getViewDatesCount(
        _view, widget.monthViewSettings.numberOfWeeksInView, widget.timeSlotViewSettings.numberOfDaysInView, nonWorkingDays);
  ...
  void _loadScheduleViewAppointmentDates(List<CalendarAppointment> collection) {
    ...
    for (int i = 0; i < collection.length; i++) {
      ...
      if (appointment.recurrenceRule != null && appointment.recurrenceRule!.isNotEmpty) {
        ...
        recursiveDates = RecurrenceHelper.getRecurrenceDateTimeCollection(appointment.recurrenceRule!, appointment.actualStartTime,
        ...
        if (appointment.recurrenceExceptionDates != null) {
          for (int i = 0; i < appointment.recurrenceExceptionDates!.length; i++) {
            recursiveExceptionDates
                .add(AppointmentHelper.convertTimeToAppointmentTimeZone(appointment.recurrenceExceptionDates![i], '', widget.timeZone));
          }
        }
      } else {
  ...
  DateTime _updateCurrentDate(CalendarView view) {
    ...
    if (_selectedDate != null && isDateWithInDateRange(visibleStartDate, visibleEndDate, _selectedDate)) {
      if (isMonthView) {
        return DateTime(_selectedDate!.year, _selectedDate!.month, _selectedDate!.day, _controller.displayDate!.hour, _controller.displayDate!.minute,
      ...
      return DateTime(
          date.year, date.month, date.day, _controller.displayDate!.hour, _controller.displayDate!.minute, _controller.displayDate!.second);
    } else {
      if (isMonthView) {
  ...
  DateTime _getMaxAppointmentDate(List<CalendarAppointment> appointments, String? timeZone, DateTime maxDate, DateTime displayDate,
  ...
  /// and schedule view settings.
  DateTime _getMinAppointmentDate(List<CalendarAppointment> appointments, String? timeZone, DateTime minDate, DateTime displayDate,
      ScheduleViewSettings scheduleViewSettings, bool useMobilePlatformUI) {
  ...
  Widget? _getItem(BuildContext context, int index, bool isRTL) {
    ...
    if (isLoadMore) {
      ...
      _minDate ??= _getMinAppointmentDate(
          _appointments, widget.timeZone, widget.minDate, scheduleDisplayDate, widget.scheduleViewSettings, _useMobilePlatformUI);
      ...
      _maxDate ??= _getMaxAppointmentDate(
          _appointments, widget.timeZone, widget.maxDate, scheduleCurrentDate, widget.scheduleViewSettings, _useMobilePlatformUI);
    ...
    bool isNeedMonthBuilder =
        _useMobilePlatformUI && (prevEndDate.month != startDate.month || prevEndDate.year != startDate.year || initialMonthHeader);
    ...
    for (int i = 0; i < dateAppointmentKeys.length; i++) {
      ...
      void addMonthHeaderView() {
        ...
        scheduleViewDetails._intersectPoint =
            currentIndex >= 0 ? previousHeight + interSectPoint + viewTopPadding : previousHeight + height - interSectPoint - viewTopPadding;
      ...
      void addDisplayOrCurrentDateView({bool isDisplayDate = true}) {
        ...
        widgets.add(_getDisplayDateView(isRTL, isDisplayDate ? scheduleDisplayDate : scheduleCurrentDate, highlightViewStartPosition, viewPadding,
            appointmentViewHeaderHeight, padding));
      ...
      currentAppointments
          .sort((CalendarAppointment app1, CalendarAppointment app2) => AppointmentHelper.orderAppointmentsAscending(app1.isAllDay, app2.isAllDay));
      currentAppointments
          .sort((CalendarAppointment app1, CalendarAppointment app2) => AppointmentHelper.orderAppointmentsAscending(app1.isSpanned, app2.isSpanned));
      ...
      widgets.add(MouseRegion(
          ...
          child: GestureDetector(
            child: _ScheduleAppointmentView(
                ...
                content: Container(
                  padding:
                      EdgeInsets.fromLTRB(isRTL ? 0 : viewPadding, appointmentViewTopPadding, isRTL ? viewPadding : 0, appointmentViewTopPadding),
    ...
    if (!isDisplayDateHighlightAdded && endDate.month != scheduleDisplayDate.month) {
      final double highlightViewStartPosition =
          currentIndex >= 0 ? previousHeight + topHeight + appointmentHeight : previousHeight + height - topHeight - appointmentHeight;
  ...
  Widget _getMonthOrWeekHeader(DateTime startDate, DateTime? endDate, bool isRTL, bool isMonthLabel,
    ...
    if (isMonthLabel && widget.scheduleViewMonthHeaderBuilder != null) {
      final ScheduleViewMonthHeaderDetails details = ScheduleViewMonthHeaderDetails(
          DateTime(startDate.year, startDate.month), Rect.fromLTWH(0, 0, _minWidth, widget.scheduleViewSettings.monthHeaderSettings.height));
    ...
    return GestureDetector(
        child: Container(
            ...
            child: RepaintBoundary(
                child: headerWidget != null
                    ...
                    : CustomPaint(
                        painter: _ScheduleLabelPainter(startDate, endDate, widget.scheduleViewSettings, isMonthLabel, isRTL, _locale,
                            _useMobilePlatformUI, _agendaViewNotifier, _calendarTheme, _themeData, _localizations, _textScaleFactor),
                        size: isMonthLabel
        ...
        onLongPressStart: (LongPressStartDetails details) {
          ...
          CalendarViewHelper.raiseCalendarLongPressCallback(widget, DateTime(startDate.year, startDate.month, startDate.day), null,
              isMonthLabel ? CalendarElement.header : CalendarElement.viewHeader, null);
        });
  }

  Widget _getDisplayDateView(bool isRTL, DateTime currentDisplayDate, double highlightViewStartPosition, double viewHeaderWidth,
        ...
        child: GestureDetector(
          child: _ScheduleAppointmentView(
              ...
              content: Container(
                ...
                child: CustomPaint(
                    painter: _ScheduleLabelPainter(currentDisplayDate, null, widget.scheduleViewSettings, false, isRTL, _locale, _useMobilePlatformUI,
          ...
          onTapUp: (TapUpDetails details) {
            ...
            _raiseCallbackForScheduleView(currentDisplayDate, details.localPosition, <CalendarAppointment>[], viewHeaderWidth, padding, true,
                isDisplayDate: true);
          },
          onLongPressStart: (LongPressStartDetails details) {
            _removeDatePicker();
            ...
            _raiseCallbackForScheduleView(currentDisplayDate, details.localPosition, <CalendarAppointment>[], viewHeaderWidth, padding, false,
                isDisplayDate: true);
          },
        ));
  }
  ...
  void _raiseCallbackForScheduleView(
      DateTime currentDate, Offset offset, List<CalendarAppointment> appointments, double viewHeaderWidth, double padding, bool isTapCallback,
  ...
  Widget addAgenda(double height, bool isRTL) {
    ...
    /// start date of the appointment collection.
    _minDate =
        _getMinAppointmentDate(_appointments, widget.timeZone, widget.minDate, currentMinDate, widget.scheduleViewSettings, _useMobilePlatformUI);
    ...
    /// end date of the appointment collection.
    _maxDate =
        _getMaxAppointmentDate(_appointments, widget.timeZone, widget.maxDate, currentMaxDate, widget.scheduleViewSettings, _useMobilePlatformUI);
    ...
    if (_nextDates.length < 10 && _previousDates.isNotEmpty) {
      ...
      List<CalendarAppointment> appointmentCollection = AppointmentHelper.getVisibleAppointments(
          viewStartDate, isSameOrBeforeDate(_maxDate, viewEndDate) ? viewEndDate : _maxDate!, _appointments, widget.timeZone, false);
      ...
      if (viewStartDate.isBefore(scheduleDisplayDate) &&
        ...
        while (visibleStartDate.isBefore(scheduleDisplayDate) && !isSameDate(visibleStartDate, scheduleDisplayDate)) {
          ...
          initialScrollPosition +=
              _getInitialScrollPosition(appStartDate, appEndDate, scheduleCurrentDate, appointmentViewHeight, allDayAppointmentHeight);
  ...
  double _getInitialScrollPosition(
      DateTime viewStartDate, DateTime viewEndDate, DateTime scheduleCurrentDate, double appointmentViewHeight, double allDayAppointmentHeight) {
            ...
            child: Container(
                ...
                child: Material(
                  ...
                  child: Scrollbar(
                      ...
                      child: ListView(padding: EdgeInsets.zero, controller: calendarViewPopupScrollController, children: children)),
                ))));
  }
  ...
  Widget _addResourcePanel(bool isResourceEnabled, double resourceViewSize, double height, bool isRTL) {
    ...
    final double resourceItemHeight =
        CalendarViewHelper.getResourceItemHeight(resourceViewSize, height - top, widget.resourceViewSettings, _resourceCollection!.length);
  ...
  Widget _addCustomScrollView(
      double top, double resourceViewSize, bool isRTL, bool isResourceEnabled, double width, double height, double agendaHeight) {
    return Positioned(
      top: top,
  ...
  Widget _addChildren(double agendaHeight, double height, double width, bool isRTL) {
    ...
    final List<Widget> children = <Widget>[
      ...
      //cho nay hien thi lich nhe fuckyouuuuuuuu
        ...
        child: _PopupWidget(
            child: Container(
                ...
                child: SfDateRangePicker(
                  ...
                  monthCellStyle:
                      DateRangePickerMonthCellStyle(textStyle: datePickerStyle, todayTextStyle: datePickerStyle.copyWith(color: todayTextColor)),
                  ...
                  onViewChanged: (DateRangePickerViewChangedArgs details) {
                    ...
                    _controller.displayDate = DateTime(selectedDate.year, selectedDate.month, selectedDate.day, _controller.displayDate!.hour,
                  ...
                  onSelectionChanged: (DateRangePickerSelectionChangedArgs details) {
                    ...
                    _showHeader = false;
                    _controller.displayDate = DateTime(details.value.year, details.value.month, details.value.day, _controller.displayDate!.hour,
                        _controller.displayDate!.minute, _controller.displayDate!.second);
                  },
                ))));
  }
  ...
  Widget _addAgendaView(double height, double startPosition, double width, bool isRTL) {
    ...
    if (agendaAppointments.isNotEmpty) {
      ...
      painterHeight = ((count * (allDayAppointmentHeight + topPadding)) + ((agendaAppointments.length - count) * (appointmentHeight + topPadding))) +
          bottomPadding;
    }
...
class _CalendarHeaderViewState extends State<_CalendarHeaderView> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.showTodayButton) {
      ...
      if (!useMobilePlatformUI) {
        ...
        final Size todayButtonSize = _getTextWidgetWidth(todayText, widget.height, widget.width - totalArrowWidth, context,
    ...
    if (isNeedViewSwitchOption) {
      ...
      if (useMobilePlatformUI) {
        ...
        for (int i = 0; i < allowedViewsLength; i++) {
          ...
          final Size calendarViewSize = _getTextWidgetWidth(_calendarViews[currentView]!, widget.height, totalWidth, context,
              style: const TextStyle(fontSize: defaultCalendarViewTextSize));
          final double currentViewTextWidth = calendarViewSize.width + allowedViewsPadding;
          maxCalendarViewHeight =
              maxCalendarViewHeight > calendarViewSize.height ? maxCalendarViewHeight : calendarViewSize.height + allowedViewsPadding;
        ...
        if (allowedViewsWidth < totalWidth) {
          ...
          for (int i = 0; i < allowedViewsLength; i++) {
            final CalendarView currentView = widget.allowedViews![i];
            children.add(_getCalendarViewWidget(headerBackgroundColor, useMobilePlatformUI, false, calendarViewsWidth[currentView]!, maxHeaderHeight,
                style, arrowColor, headerTextColor, currentView, widget.view == currentView, highlightColor, defaultCalendarViewTextSize));
          }
        } else {
          ...
          children.add(_getCalendarViewWidget(headerBackgroundColor, useMobilePlatformUI, true, calendarViewWidth, maxHeaderHeight, style, arrowColor,
              headerTextColor, widget.view, widget.viewChangeNotifier.value, highlightColor, defaultCalendarViewTextSize,
              semanticLabel: 'CalendarView'));
        }
      }
    }
    ...
    if (weekNumberEnabled) {
      ...
      final double remainingWidth =
          widget.width - calendarViewWidth - todayIconWidth - dividerWidth - totalArrowWidth - weekNumberPanelWidth - headerWidth;
      if (remainingWidth < 0) {
  ...
  Widget _getCalendarViewWidget(Color headerBackgroundColor, bool useMobilePlatformUI, bool isNeedIcon, double width, double height, TextStyle style,
      Color arrowColor, Color headerTextColor, CalendarView view, bool isHighlighted, Color? highlightColor, double defaultCalendarViewTextSize,
  ...
  String _getHeaderText() {
    ...
    final int visibleDatesCount =
        DateTimeHelper.getViewDatesCount(widget.view, widget.numberOfDaysInView, widget.numberOfDaysInView, widget.nonWorkingDays);
    switch (widget.view) {
      case CalendarView.schedule:
        {
...
class _ScheduleLabelPainter extends CustomPainter {
  _ScheduleLabelPainter(this.startDate, this.endDate, this.scheduleViewSettings, this.isMonthLabel, this.isRTL, this.locale, this.useMobilePlatformUI,
      this.agendaViewNotifier, this.calendarTheme, this.themeData, this._localizations, this.textScaleFactor,
  ...
  void _addDisplayDateLabel(Canvas canvas, Size size) {
    ...
    final TextSpan span = TextSpan(
      ...
      style:
          themeData.textTheme.bodyMedium!.merge(scheduleViewSettings.weekHeaderSettings.weekTextStyle ?? scheduleViewSettings.placeholderTextStyle),
    );
    ...
    if (isDisplayDate && agendaViewNotifier.value != null && isSameDate(agendaViewNotifier.value!.hoveringDate, startDate)) {
      ...
      if (useMobilePlatformUI) {
        ...
        final Rect rect =
            Rect.fromLTWH(0, padding + viewPadding, size.width - (isRTL ? viewPadding : padding), size.height - (2 * (viewPadding + padding)));
...
class _AgendaDateTimePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (monthViewSettings != null) {
      ...
      if (useMobilePlatformUI) {
        ...
        dateTextStyle =
            agendaDateThemeTextStyle.merge(calendarTheme.agendaDateTextStyle).merge(scheduleViewSettings!.dayHeaderSettings.dateTextStyle);
      } else {
  ...
  void _addDayLabelForMobile(Canvas canvas, Size size, double padding, TextStyle dayTextStyle, TextStyle dateTextStyle, bool isToday, bool isMobile) {
    ...
    if (agendaDateNotifier.value != null && isSameDate(agendaDateNotifier.value!.hoveringDate, selectedDate)) {
      if (xPosition < agendaDateNotifier.value!.hoveringOffset.dx &&
        ...
        _linePainter.color =
            isToday ? Colors.black.withOpacity(0.1) : (themeData.brightness == Brightness.dark ? Colors.white : Colors.black87).withOpacity(0.04);
        _drawTodayCircle(canvas, xPosition, yPosition, padding);
      }
    }
  ...
  void _addDayLabelForWeb(Canvas canvas, Size size, double padding, TextStyle dayTextStyle, TextStyle dateTextStyle, bool isToday) {
    ...
    final String dayText = DateFormat(
            isRTL ? '${scheduleViewSettings!.dayHeaderSettings.dayFormat}, MMM' : 'MMM, ${scheduleViewSettings!.dayHeaderSettings.dayFormat}', locale)
        .format(selectedDate!)
        .toUpperCase();
...
Size _getTextWidgetWidth(String text, double height, double width, BuildContext context, {TextStyle? style}) {
  ...
  return Size(textPainter.width + 10, textPainter.height + 10);
